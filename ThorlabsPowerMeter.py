...
class ThorlabsPowerMeter():
    ...
    def disconnect(self):
        if self.isConnected:
            self.logger.info(f'|{self.__class__.__name__}| Disconnecting connected device {self.resourceNameConnected}')
            try:
                self.deviceNET.Dispose()
                self.isConnected=False
                self.logger.info(f'|{self.__class__.__name__}| Device {self.resourceNameConnected} released properly')
            except:
                self.logger.warning(f'|{self.__class__.__name__}| Unable to disconnect device.')
        else:
            self.logger.warning(f'|{self.__class__.__name__}| Device not connected.')
    ...
    def setAverageTime(self,averageTime):
        ...
        if self.averageTimeMin<=averageTime and averageTime<=self.averageTimeMax:
            ...
            if self.averageTimeMin>averageTime:
                self.logger.warning(f'|{self.__class__.__name__}| Exceed minimum average time! Force to the minimum.')
                averageTime=self.averageTimeMin
            if averageTime>self.averageTimeMax:
                self.logger.warning(f'|{self.__class__.__name__}| Exceed maximum average time! Force to the maximum.')
                averageTime=self.averageTimeMax
                
        self.deviceNET.setAvgTime(averageTime)
        self.averageTimeSet=averageTime
        self.logger.info(f'|{self.__class__.__name__}| Average time set to {averageTime}')
           
    def getAverageTime(self):
        self.averageTimeSet=self.deviceNET.getAvgTime(0)
        self.logger.info(f'|{self.__class__.__name__}| Average time set to {self.averageTimeSet}')     
            
    def setTimeoutValue(self,timeout):
        self.deviceNET.setTimeoutValue(timeout)
        self.timeoutValue=timeout
        self.logger.info(f'|{self.__class__.__name__}| Timeout value set to {self.timeoutValue}')
    
    def getTimeoutValue(self):
        _,self.timeoutValue=self.deviceNET.getTimeoutValue()
        self.logger.info(f'|{self.__class__.__name__}| Timeout value is {self.timeoutValue}')
    ...
    def setWaveLength(self,wavelength):
        ...
        if (self.wavelengthMin<=wavelength and wavelength<=self.wavelengthMax):
            ...
            if self.wavelengthMin>wavelength:
                self.logger.warning(f'|{self.__class__.__name__}| Exceed minimum wavelength! Force to the minimum.')
                wavelength=self.wavelengthMin
            if wavelength>self.wavelengthMax:
                self.logger.warning(f'|{self.__class__.__name__}| Exceed maximum wavelength! Force to the maximum.')
                wavelength=self.wavelengthMax
                
        self.deviceNET.setWavelength(wavelength)
        self.wavelengthSet=wavelength
        self.logger.info(f'|{self.__class__.__name__}| Set wavelength to {wavelength}')
        
    def getWaveLength(self):
        _,self.wavelengthSet=self.deviceNET.getWavelength(0)
        self.logger.info(f'|{self.__class__.__name__}| Wavelength was set to {self.wavelengthSet}')

    def setPowerAutoRange(self,enable:bool):
        self.deviceNET.setPowerAutoRange(enable)
        self.logger.info(f'|{self.__class__.__name__}| Set auto power range')
    ...
    def setPowerRange(self,maxRange):
        ...
        if (self.powerRangeMin<=maxRange and maxRange<=self.powerRangeMax):
            ...
            if self.powerRangeMin>maxRange:
                self.logger.warning(f'|{self.__class__.__name__}| Exceed minimum range! Force to the minimum.')
                maxRange=self.powerRangeMin

            if maxRange>self.powerRangeMax:
                self.logger.warning(f'|{self.__class__.__name__}| Exceed maximum range! Force to the maximum.')
    ...
    def setDispBrightness(self,brightness):
        ...
        if (self.brightnessMin<=brightness and brightness<=self.brightnessMax):
            ...
            if self.brightnessMin>brightness:
                self.logger.warning(f'|{self.__class__.__name__}| Exceed minimum brightness! Force to the minimum.')
                brightness=self.brightnessMin
            if brightness>self.brightnessMax:
                self.logger.warning(f'|{self.__class__.__name__}| Exceed maximum brightness! Force to the maximum.')
                brightness=self.brightnessMax

        self.deviceNET.setDispBrightness(brightness)
        self.brightnessSet=brightness
        self.logger.info(f'|{self.__class__.__name__}| Set Display brightness to {brightness}')
        
    def getDispBrightness(self):
        self.brightnessSet=self.deviceNET.getDispBrightness()
        self.logger.info(f'|{self.__class__.__name__}| Display brightness was set to {self.brightnessSet}')
    ...
    def setAttenuation(self,attenuation):
            if self.modelName in {'PM100D', 'PM100A', 'PM100USB', 'PM200', 'PM400'}:
                ...
                self.logger.info(f'|{self.__class__.__name__}| Set Display brightness to {attenuation}')
            else:
    ...
    def getSensorInfo(self):
        ...
        match _type:
            case 0x00:
                ...
                match _subtype:
                    ...
                    case _:
                        self.logger.warning(f'|{self.__class__.__name__}| Unknown sensor.')
            case 0x01:
        ...
        self.logger.info(f'|{self.__class__.__name__}| Sensor info:')
        self.logger.info(f'|{self.__class__.__name__}| |--> Name: {self.sensorName}')
        self.logger.info(f'|{self.__class__.__name__}| |--> S/N: {self.sensorSerialNumber}')
        self.logger.info(f'|{self.__class__.__name__}| |--> Type: {self.sensorType}')
        self.logger.info(f'|{self.__class__.__name__}| |--> SubType: {self.sensorSubType}')
        self.logger.info(f'|{self.__class__.__name__}| |--> Flag: {self.sensorFlags}')
    ...
    def updatePowerReading(self,period:float=0.5):
        ...
        match _unit:
            ...
            case _:
                self.logger.warning(f'|{self.__class__.__name__}| Unknown unit')
    ...
    def listDevices(cls,libraryPath:str=defaultPath):
        ...
        try:
            ...
            cls.logger.info(f'|{cls.__name__}| ThorlabsPowerMeter Driver Successfully Loaded.')
            cls.logger.info(f'|{cls.__name__}| ThorlabsPowerMeter Driver version: {cls.driverVersion}.')
            
        except:
            cls.logger.error(f'|{cls.__name__}| Unable to load ThorlabsPowerMeter Driver.')
        ...
        if cls.resourceCount<=0:
            cls.logger.warning(f'|{cls.__name__}| No device found.')
        else:
            cls.logger.info(f'|{cls.__name__}| {cls.resourceCount} device(s) are found.')
            for i in range(cls.resourceCount):
                ...
                cls.logger.info(f'|{cls.__name__}| Found resource {_description[0].ToString()}.')
        del _tempInstance,_description
        cls.logger.info(f'|{cls.__name__}| Device scan is completed.')
        return cls()
